export const outside = `der Bogen, die Bogen	arch
der Backstein, die Backsteine	brick
das Gebäude, die Gebäude	building
das Schloss, die Schlösser	castle
die Kirche, die Kirchen	church
der Fahrstuhl, die Fahrstühle	elevator (travel chair)
die Fabrik, die Fabriken	factory
der Bauernhof, die Bauernhöfe	farm
der Hydrant, die Hydranten	fire hydrant
die Fahne, die Fahnen	flag
der Hügel, die Hügel	hill
das Heim, die Heime	home
das Haus, die Häuser	house
der Leuchtturm, die Leuchttürme	lighthouse
der Markt, die Märkte	market
der Berg, die Berge	mountain
der Schlamm, die Schlämme	mud
das Museum, die Museen	museum
der Nachbarschaft, die Nachbarschaften	neighborhood
der Palast, die Paläste	palace
die Ebene, die Ebenen	plain (flatland)
der Teich, die Teiche	pond
die Pfütze, die Pfützen	puddle
der Fluss, die Flüsse	river
die Schule, die Schulen	school
der Gehsteig, die Gehsteige	sidewalk
der Wolkenkratzer, die Wolkenkratzer	skyscraper
die Statue, die Statuen	statue
der Strom, die Ströme	stream (also electrical current)
die Stadt, die Städte	town
das Rathaus, die Rathäuser	townhall
das Dorf, die Dörfer	village
der Wasserfall, die Wasserfälle	waterfall
die Windmühle, die Windmühlen	windmill
der Deich, die Deiche	dike
der Damm, die Dämme	dam
das Kreuz, die Kreuze	cross
die Ecke, die Ecken	corner
der Dom, die Dome	cathedral
der Hof, die Höfe	yard (as in courtyard)
die Ernte, die Ernten	harvest
das Heu	hay`;
